...
{
    public  class ByteMesRepository
    {
        ...
        public ByteMesRepository(string ip, int port)
        {
            try
            {
        ...
        public async  Task<ByteMESCheck> Send(ByteMESCheck entity, string CH)
        {
            ...
            try
            {
                ...
                packet.AddRange(ASCIIEncoding.ASCII.GetBytes(entity.Config));
        ...
        public  async Task<ByteMESCheck> SendLogin(ByteMESCheck entity, string ch)
        {
            this.isReceiver = false;
            try
            {  
        ...
        private ByteMESCheck FilterData(string data, ByteMESCheck mesOld, string CH)
        {
            try
            {
                ByteMESCheck newMESCheck = new ByteMESCheck();
                int idex = 0;
                // EQUIPMENT ID
                ...
                idex += 10;
                // Config
                string config = data.Substring(idex, 40);
                //if (mesOld.Config != config)
                //{
                //    logger.Create($@"Config Is Diffrent {CH}: Old('" + mesOld.Config + "') , New('" + config + "')", LogLevel.Warning);
                //    return null;
                //}
                logger.Create($@"MES.RECEIVER Config {CH}:" + config, LogLevel.Information);
                newMESCheck.Config = lotNo;
                idex += 40;
                //BODY
                idex += 1;
